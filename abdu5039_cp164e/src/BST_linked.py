...
class BST:
    ...
    def _flip_aux(self, node):
        ...
        if node is not node:
            ...
            # Recursively flip right subtree
    ...
    def _are_flipped_aux(self, node1,node2):
        ...
        -------------------------------------------------------
        Parameters:
            node1 - the root node of the first subtree (Node)
            node2 - the root node of the second subtree (Node)
        Returns‌​‌​​​​‌​​‌‌‌​‌‌‌​‌​​‌​​‌‌‌‌:
            flipped - True if right is a flipped version of left, False otherwise.
        -------------------------------------------------------
        """
        # Checks if both nodes are none
        if node1 is None and node2 is None:
            return  True
        # If one of the node is flippes while the other is not, they are not flipped
        if node1 is None or node2 is None:
            return False
        # This will check all the other values are the same and the subtrees are flipped
        return (node1._value == node2._value and 
                self._are_flipped_aux(node1._left, node2._right) and 
    ...
    def _total_height_aux(self, node):
        ...
        ---------------------------------------------------------
        Parameters:
            node - the root node of the subtree for that is used to calcualte the total height (Node)
        Returns‌​‌​​​​‌​​‌‌‌​‌‌‌​‌​​‌​​‌‌‌‌:
            total_height - the total height count - i.e. the sum of all the
            node heights in the tree (int)
        ---------------------------------------------------------
        """
        # This is to check if the current node is NONE
        if node is None:
            # If True it will return 0
            return 0
        else:
            # This will get the total height of the left subtree
            left_height = self._total_height_aux(node._left)
            # This will get the total height of the right subtree
            right_height = self._total_height_aux(node._right)
            # This will return the height of the current node and the heights returned by the recursive calls.
            return node._height + left_height + right_height
    ...
    def furthest(self):
        ...
        -------------------------------------------------------
        """
        # This will check to see if the root of the tree is NONE
        if self._root is None:
            # This will return an empty list if there is no root
            return []
     
            # else it will find the values of the left-most node by passing "left" as the direction for the furtest method
            leftmost = self.furthest(self._root, 'left')._value
            # else it will find the values of the right-most node by passing "right" as the direction for the furtest method
            rightmost = self._furthest(self._root, 'right')._value
            # This will return the value of the left most and the right most node in the list. 
